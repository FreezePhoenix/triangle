...
#ifdef TRILIBRARY
  const unsigned int* triangle_ptr = trianglelist.get();
#endif
  while (triangleloop.tri != (triangle*)NULL) {
#ifdef TRILIBRARY
...
#ifdef TRILIBRARY
  unsigned int* triangle_ptr = tlist.get();
...
#ifdef ANSI_DECLARATORS
void triangulate(const char* triswitches, std::shared_ptr<triangulateio> in, std::shared_ptr<triangulateio> out,
